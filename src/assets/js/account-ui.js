...
async function boot() {
  ...
  // ✅ Essential change: get a default session token (no template)
  const token = await window.Clerk.session.getToken().catch(() => "");

  if (!token) {
    if (messagesEl) {
      messagesEl.innerHTML = "";
      addMessage(messagesEl, "Signed in, but couldn’t establish a secure session token.", "warning");
    }
  ...
  try {
    // ✅ Essential change: always send Authorization header
    ...
    if (res.status === 401) {
      ...
      if (messagesEl) {
        messagesEl.innerHTML = "";
        addMessage(messagesEl, `Signed in, but authentication was rejected${reason}.`, "warning");
      }
    ...
    // Backend no longer needs to return email; keep your UI behaviour stable.
